...
// =====================================================
// REMINDER ENGINE (Option B .env names)
// Sends once/day ONLY AFTER due_date passed (due_date < today)
// Joins pickups.customer_id -> customers.id to get name/phone
// Uses pickups.last_sent_on to avoid same-day duplicates
// =====================================================
...
const REMINDER_CRON = process.env.REMINDER_CRON || "0 9 * * *"; // default 9am

// Option B: match your current .env names exactly
const TWILIO_SID = process.env.accountSid;
const TWILIO_AUTH = process.env.authToken;
...
const stmtMarkSent = db.prepare(`
  UPDATE pickups
  SET last_sent_on = ?, last_reminder_sent = ?
  WHERE id = ?
`);

function runReminderJob() {
  if (!RUN_REMINDERS) {
    console.log("⏸ RUN_REMINDERS is not true — skipping reminders.");
    return;
  }
  ...
  for (const r of rows) {
    const dueKey = normalizeDateKey(r.due_date);
    if (!dueKey) continue;

    // ✅ only AFTER due date passed (not before, not on due date)
    if (!(dueKey < today)) continue;

    overdue++;

    // ✅ once per day max
    if (r.last_sent_on === today) {
      skippedSameDay++;
      continue;
    }
    ...
    // ✅ dry-run safe mode
    if (!SEND_SMS) {
      console.log(`🧪 (SEND_SMS=false) Would send to ${phone}: ${smsBody}`);
      continue;
    }

    // ✅ real send mode
    if (!twilioClient || !TWILIO_FROM) {
      console.error("❌ Twilio not configured (accountSid/authToken/TWILIO_NUMBER).");
      continue;
    }
    ...
    twilioClient.messages
      .create({ body: smsBody, from: TWILIO_FROM, to: phone })
      .then(() => {
        stmtMarkSent.run(today, today, r.pickup_id);
      })
      .catch((e) => {
        console.error(`❌ SMS failed for pickup ${r.pickup_id}:`, e?.message || e);
      });
  }

  console.log(`🔔 Reminder job done. overdue=${overdue}, attempted=${attempted}, skippedSameDay=${skippedSameDay}`);
}

// Cron schedule (from .env REMINDER_CRON or default)
cron.schedule(REMINDER_CRON, () => runReminderJob());

// Manual trigger for testing (blocked if SEND_SMS=true)
app.post("/admin/run-reminders", (req, res) => {
  if (SEND_SMS) return res.status(403).json({ ok: false, error: "Manual trigger disabled when SEND_SMS=true" });
  runReminderJob();
